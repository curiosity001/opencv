...
rtmpUrl1 = "rtmp://192.168.65.201:1935/live/4"
rtmpUrl2 = "rtmp://192.168.65.201:1935/live/5"
rtmpUrl3 = "rtmp://192.168.65.201:1935/live/6"
...
def image_inference(frame, model, device, imgsz, names, colors):
    t1 = time.time()
    face_status = []
    # frame = frame[roi[0]:roi[2], roi[1]:roi[3]]
    ...
    def run(self):
        while True:
            try:
                ...
                self.q.put(self.frame if self.frame is not None else np.zeros(shape=(1080, 1920, 3), dtype=float))
...
class Video_analyze_thread(threading.Thread):
    def __init__(self, threadID, name, thread_num, frame_name, img_infer, model, queue, pipe, camera_id, region1):
        threading.Thread.__init__(self)
                        ...
                        if self.fatigue_timer1 == 0:
                            ...
                            if self.face_tired_list != self.face_tired_list_before:
                                if self.face_tired_list == [1]:
                                    self.udp_socket.sendto(f"{self.region1}_{self.face_tired}".encode('utf-8'), udp_addr)
...
#视频接收线程(one people)
    ...
    thread_read3 = Video_receive_thread(3, "video_read3", url3, 'frame3',video_queue3)
...
#离岗/疲劳分析线程（一个摄像头检测一个人）
    thread_ana1 = Video_analyze_thread(4, "video_ana1", thread_read1, 'frame1', image_inference, model, video_queue1, pipe1, 65, region[0])
    thread_ana2 = Video_analyze_thread(5, "video_ana2", thread_read2, 'frame2', image_inference, model, video_queue2, pipe2, 139, region[1])
    thread_ana3 = Video_analyze_thread(6, "video_ana3", thread_read3, 'frame3', image_inference, model, video_queue3, pipe3, 140, region[2])
