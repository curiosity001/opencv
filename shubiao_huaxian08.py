...
class CameraApp(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.camera_ip = QLineEdit("192.168.31.61")
    ...
    # 在 CameraApp 类中修改 update_camera 方法
    ...
    def update_camera(self, frame):
        try:
            ...
            print("Error in update_camera:", e)
    ...
    def process_coordinate(self, coordinate):
        ...
        # 获取原始图像的宽高
        orig_img_width = self.camera_thread.cap.get(cv2.CAP_PROP_FRAME_WIDTH)
        orig_img_height = self.camera_thread.cap.get(cv2.CAP_PROP_FRAME_HEIGHT)

        # 计算缩放比例
        scale_x = orig_img_width / img_width
        scale_y = orig_img_height / img_height

        # 转换坐标点到原始图像坐标系
        orig_x = int(x * scale_x)
        orig_y = int(y * scale_y)

        points123.append((orig_x, orig_y))

        print("Processed coordinate:", (orig_x, orig_y))
        print(points123)
