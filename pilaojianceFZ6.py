...
#视频接收线程（一个摄像头有两个人的视频分割）
class Video_receive_thread1(threading.Thread):
    def __init__(self, threadID, name, url, frame_name1,frame_name2):
        threading.Thread.__init__(self)
        self.threadID = threadID
        self.name = name
        self.url = url
        self.frame_name = frame_name1
        self.frame_name = frame_name2
        self.frame = 0
        self.frame1 = 0
        self.frame2 = 0
        self.cap = cv2.VideoCapture(self.url)
    def run(self):
        while True:
            ...
            self.frame1 = self.frame[0:480,0:320]
            self.frame2 = self.frame[0:480,320:640]
...
thread_read2 = Video_receive_thread1(2, "video_read2", url1, 'frame1','frame2')
...
class Video_analyze_thread(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            self.frame_read, self.sleep_getaway , self.face_tired = self.img_infer(self.thread_num.frame,
                                                                                     self.model1,
...
class Video_analyze_thread1(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            self.frame_read1, self.sleep_getaway1, self.face_tired1 = self.img_infer(self.thread_num.frame1, self.model1,
                                                                                  device, imgsz, names, colors)
            self.frame_read2, self.sleep_getaway2, self.face_tired2 = self.img_infer(self.thread_num.frame2, self.model1,
                                                                                  device, imgsz, names, colors)

            #左边人员的在岗/离岗状态判断（frame1）
            ...
            if self.sleep_getaway2 == 1:
                if self.fatigue_timer2 == 0:
                    ...
                    if self.time_away1 >= 5:
                        self.frame_read2 = cv2AddChineseText(self.frame_read2, "睡着/离开", (50, 70), (255, 0, 0), 50)
            ...
            if self.face_tired2 == 1:
                if self.fatigue_timer3 == 0:
                    ...
                    if self.time_tired1 >= 4:
                        self.frame_read2 = cv2AddChineseText(self.frame_read2, "疲劳", (50, 70), (255, 0, 0), 50)
            ...
            self.frame3 = np.concatenate((self.frame_read1,self.frame_read2),axis=1)
            cv2.imshow(self.frame_name, self.frame3)
            if cv2.waitKey(1) & 0xFF == ord('q'):
                break
        cv2.destroyAllWindows()
...
if __name__ == "__main__":
    ...
    thread_ana2 = Video_analyze_thread1(9, "video_ana2", thread_read2, 'frame1&2', image_inference, model)
    #thread_read1.start()
    thread_read2.start()
    time.sleep(2)
    #thread_ana1.start()
