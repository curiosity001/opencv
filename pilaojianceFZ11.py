...
#
push_url1 = "rtsp://192.168.31.240:554/room7"
push_url2 = "rtsp://192.168.31.240:554/room8"
...
command = ['ffmpeg',  # linux不用指定
    # '-hwaccel','cuvid',  # 适用cuvid硬件加速解码
    '-y', '-an',  # -y 表示（可选）覆盖输出文件（如果存在）     -an  表示告诉FFMPEG不要期待任何音频（静音）
    '-f', 'rawvideo',  # 输出格式-f/-format需要是rawvideo，即完整的RGB或YUV信息.rawvideo是只有颜色数据的，所以需要指定宽高
    '-vcodec','rawvideo',  # 设置视频编解码器，视频编码器为原始视频。这是-codec:v的别名
    '-pix_fmt', 'bgr24',  # 输入像素格式
    '-s', "{}x{}".format(width, height),  # 帧尺寸，宽和高
    '-r', str(fps),  # 帧率。自己的摄像头的fps是0，若用自己的notebook摄像头，设置为15、20、25都可。
    '-i', '-',  # 表示输入是管道
    '-c:v', 'h264_nvenc',  # 视频编码方式为h264_nvenc
    '-pix_fmt', 'yuv420p',  # 输出像素格式
    #'-preset', 'ultrafast',
    '-f', 'rtsp',  # 输出视频格式，通常自己指定（flv rtsp）
    '-rtsp_transport', 'tcp',  # 使用TCP推流，linux中一定要有这行（适用TCP传输协议）
    push_url1] # 推送的目标URL（rtsp rtmp）
...
class Video_analyze_thread(threading.Thread):
    def __init__(self, threadID, name, thread_num, frame_name, img_infer, model, queue, pipe, region1, region2):
        threading.Thread.__init__(self)
...
class Video_analyze_thread1(threading.Thread):
    def __init__(self, threadID, name, thread_num, frame_name, img_infer, model, queue, pipe, region1,  region2, region3, region4):
        threading.Thread.__init__(self)
...
#视频接收线程(one people)
    ...
    thread_read1 = Video_receive_thread(1, "video_read1", url0, 'frame1',video_queue1)
...
#离岗/疲劳分析线程（一个摄像头检测一个人）
    thread_ana1 = Video_analyze_thread(6, "video_ana1", thread_read1, 'frame1', image_inference, model, video_queue1, pipe1, region[0], region[3])
...
#离岗/疲劳分析线程（一个摄像头检测两个人）
    # thread_ana6 = Video_analyze_thread1(11, "video_ana6", thread_read1, 'frame6', image_inference_2, model, video_queue2, pipe2, region[1], region[4], region[2], region[5])
    # thread_ana7 = Video_analyze_thread1(12, "video_ana7", thread_read2, 'frame7', image_inference_2, model, video_queue2, pipe2, region[1], region[4], region[2], region[5])
