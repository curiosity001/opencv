...
#摄像头选择/域名
url0 = 0
# url1='rtsp://admin:Aust12345@192.168.31.59:554/live'
# url2='rtsp://admin:Aust12345@192.168.31.60:554/live'
# url3='rtsp://admin:Aust12345@192.168.31.62:554/live'
# url4='rtsp://admin:Aust12345@192.168.31.63:554/live'
# #url5='rtsp://admin:Aust12345@192.168.31.66:554/live' # 不能用
# url6='rtsp://admin:Aust12345@192.168.31.67:554/live'
# url7='rtsp://admin:Aust12345@192.168.31.68:554/live'
# url8='rtsp://admin:Aust12345@192.168.31.92:554/live'
...
region = ["J1_L00", "J2_L02", "J2_L03","J1_P00", "J2_P04", "J2_P05", "J2_P06"]
...
class Video_analyze_thread(threading.Thread):
    def __init__(self, threadID, name, thread_num, frame_name, img_infer, model, queue,region1, region2):
        threading.Thread.__init__(self)
    ...
    def run(self):
        while True:
            ...
            if self.q.qsize() == 0:
                ...
                if self.count == 20:
                    self.count = 0
                    self.region = [self.region1,self.region2]
                    for i in range(len(self.region)):
                        self.udp_socket.sendto(f"{self.region[i]}_{self.Camera_drop_off}".encode('utf-8'),
                                           udp_addr)
            #当摄像头正常运行时，执行疲劳分析代码
            else:
                self.count=0
...
class Video_analyze_thread1(threading.Thread):
    def __init__(self, threadID, name, thread_num, frame_name, img_infer, model, queue,region1, region2, region3, region4):
        threading.Thread.__init__(self)
    ...
    def run(self):
        while True:
            ...
            if self.q.qsize() == 0:
                ...
                if self.count == 20:
                    self.count = 0
                    self.region = [self.region1, self.region2, self.region3, self.region4]
                    for i in range(len(self.region)):
                        self.udp_socket.sendto(f"{self.region[i]}_{self.Camera_drop_off}".encode('utf-8'),
                                               udp_addr)
            else:
                ...
                self.frame_read1, self.sleep_getaway1, self.face_tired1, self.sleep_getaway2, self.face_tired2 = \
                    self.img_infer(self.q.get(), self.model1, device, imgsz, names, colors, 960)
...
#视频接收线程（一个摄像头检测两个人）
    ...
    # thread_read1 = Video_receive_thread(1, "video_read1", url1, 'frame1',video_queue2)
...
#离岗/疲劳分析线程（一个摄像头检测一个人）
    thread_ana0 = Video_analyze_thread(6, "video_ana0", thread_read0, 'frame0', image_inference, model, video_queue1,region[0], region[3])
    # thread_ana1 = Video_analyze_thread(6, "video_ana1", thread_read1, 'frame1', image_inference, model, video_queue1,region[0], region[3])
    # thread_ana2 = Video_analyze_thread(7, "video_ana2", thread_read2, 'frame2', image_inference, model, video_queue1,region[0], region[3])
    # thread_ana3 = Video_analyze_thread(8, "video_ana3", thread_read3, 'frame3', image_inference, model, video_queue1,region[0], region[3])
    # thread_ana4 = Video_analyze_thread(9, "video_ana4", thread_read4, 'frame4', image_inference, model, video_queue1,region[0], region[3])
    # thread_ana5 = Video_analyze_thread(10, "video_ana5", thread_read5, 'frame5', image_inference, model, video_queue1,region[0], region[3])


#离岗/疲劳分析线程（一个摄像头检测两个人）
    # thread_ana6 = Video_analyze_thread1(11, "video_ana6", thread_read1, 'frame6', image_inference_2, model,video_queue2,region[1], region[4], region[2], region[5])
    # thread_ana7 = Video_analyze_thread1(12, "video_ana7", thread_read2, 'frame7', image_inference_2, model,video_queue2,region[1], region[4], region[2], region[5])
    # thread_ana8 = Video_analyze_thread1(13, "video_ana8", thread_read3, 'frame8', image_inference_2, model,video_queue2,region[1], region[4], region[2], region[5])
    # thread_ana9 = Video_analyze_thread1(14, "video_ana9", thread_read4, 'frame9', image_inference_2, model,video_queue2,region[1], region[4], region[2], region[5])
    # thread_ana10 = Video_analyze_thread1(15, "video_ana10", thread_read5, 'frame10', image_inference_2, model,video_queue2,region[1], region[4], region[2], region[5])
...
#视频显示
    cv2.namedWindow(thread_ana0.frame_name, cv2.WINDOW_NORMAL)
    ...
    while True:
        cv2.imshow(thread_ana0.frame_name, thread_ana0.frame_read)
